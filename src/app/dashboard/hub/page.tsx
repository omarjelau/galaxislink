...
export default function HubPage() {
  ...
  const maxLinkPages = { free: 2, basic: 5, premium: Infinity, enterprise: Infinity }[userPlan as 'free' | 'basic' | 'premium' | 'enterprise'] || 2;
  const maxCards = { free: 1, basic: 1, premium: 4, enterprise: Infinity }[userPlan as 'free' | 'basic' | 'premium' | 'enterprise'] || 1;

  const canAddMoreLinkPages = (linkPages?.length || 0) < maxLinkPages;
  ...
  const handleCreateNew = async () => {
    if (!user || !newPageName) {
     ...
     if (newPageType === 'linkPage' && !canAddMoreLinkPages) {
